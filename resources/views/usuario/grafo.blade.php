...
document.addEventListener('DOMContentLoaded', function() {
    modal = new bootstrap.Modal(document.getElementById('modalAutor'));
});
...
function abrirModal(autor) {
    ...
    while (g < generos.length) {
        generosHtml += '<span class="badge" style="background:var(--vino);color:#fff;">' + generos[g] + '</span>';
        g++;
    }
    ...
    while (j < conexiones.length) {
        ...
        var imgHtml = fotoDestino
            ? '<img src="/storage/' + fotoDestino + '" style="width:45px;height:45px;object-fit:cover;border-radius:50%;border:2px solid var(--vino);flex-shrink:0;">'
        ...
        relacionadosHtml +=
            ...
            imgHtml +
            '<div><div style="font-weight:600;color:#3d0f0f;">' + destino + '</div>' +
