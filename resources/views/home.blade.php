...
@php
    ...
    $librosData = $librosHome->map(function($l) {
        return [
            'id'        => $l->id,
            'titulo'    => $l->titulo,
...
<nav class="navbar navbar-expand-sm navbar-home py-2">
    <div class="container">
        ...
        <div class="ms-auto d-flex align-items-center gap-2">
            ...
            @else
                <a href="{{ url('/login') }}?redirect={{ urlencode(url('/carrito')) }}"
...
<script>
    ...
    function mostrarLibro() {
        ...
        while (i < libros.length) {
            if (libros[i].autor === libro.autor) {
                librosDelAutor.push(i);
            }
            i++;
        }
        ...
        if (librosDelAutor.length <= 1) {
            listaHtml += '<p style="font-size:0.78rem;color:#888;margin:0;">No hay otros libros de este autor en el catálogo.</p>';
        } else {
            var j = 0;
            while (j < librosDelAutor.length) {
                ...
                var borderLeft = esActual ? 'border-left:3px solid #6B1C1C;' : '';
                ...
                listaHtml +=
                    '<div onclick="cambiarLibro(' + idx2 + ')" style="display:flex;align-items:center;gap:10px;padding:6px 8px;border-radius:6px;cursor:pointer;background:' + bgColor + ';' + borderLeft + 'margin-bottom:4px;">' +
                    imgHtml +
                    '<div>' +
                    '<div style="font-size:0.82rem;font-weight:600;color:#3d0f0f;">' + libros[idx2].titulo + '</div>' +
                    '<div style="font-size:0.75rem;color:#6B1C1C;font-weight:600;">' + libros[idx2].autor + '</div>' +
                    '</div></div>';
                j++;
            }
        }
        ...
        if (otrosCount > 0) {
            listaHtml += '<p style="font-size:0.78rem;color:#888;margin-top:8px;margin-bottom:0;">+ ' + otrosCount + ' libro(s) de otros autores disponibles.</p>';
        }
        ...
        if (esUsuario) {
            ...
            accionesHtml =
                '<a href="/libros/' + libro.id + '/edit" style="background:#ffc107;color:#000;border:none;border-radius:6px;padding:10px 20px;text-decoration:none;display:inline-block;margin-right:8px;">Editar</a>' +
            ...
            accionesHtml =
                '<a href="' + loginUrl + '" style="background:#6B1C1C;color:#fff;border:none;border-radius:6px;padding:10px 20px;text-decoration:none;display:block;text-align:center;">Inicia sesion para comprar</a>';
        }
